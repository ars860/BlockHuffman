...
encode_report encode_decode_test(string &&test_message) {
    auto[encoded, report] = encode(test_message, true);
...
void write_read_file_test() {
    ...
    string test_message = "IF_WE_CANNOT_DO_AS_WE_WOULD_WE_SHOULD_DO_AS_WE_CAN";
    auto[encoded, report] = encode(test_message, true);
    write_to_file(test_message, "test.txt");

    string from_file = read_file_to_string("test.txt");
    string decoded_from_file = decode(from_file);
    string decoded = decode(encoded);
...
int main() {
    ...
    bool csv = true;
    if (csv) {
        cout << encode_report::csv_header << '\n';
        csv_report += encode_report::csv_header;
        csv_report += '\n';
    }
